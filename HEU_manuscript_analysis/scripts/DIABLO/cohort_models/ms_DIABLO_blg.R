###################
# Nelly Amenyogbe
# HEU Manuscript Analysis: Prepare data matrices for DIABLO:BELGIUM
###################

# In this script, we will perform DIABLO intgration for Luminex-cytokine data and OTU data for the BELGIAN cohort.
...
X.otu <- spls.dat$BLG$Xotu
Y.lmx <- log10(spls.dat$BLG$Ylmx)
meta <- spls.dat$BLG$meta
Y = meta$Group
...
# tune diablo ####
# set n to test
test.keepX = list (otu = c(5:9, seq(10, 22, 2)),
                   lmx = c(5:9, seq(10, 22, 2)))
# run tune.0
tune.blg.0 <- tune.block.splsda(X = dat, 
                                Y = Y,
                                ncomp = 2,
...
choice.keepx.0 <- tune.blg.0$choice.keepX # this varies slightly.  Here we select the number passed to the final analysis.
choice.keepx.0 <- list("otu" = c(5, 7), "lmx" = c(6, 6))
...
error <- perf(res.final, validation = "Mfold", folds = 2)
error$error.rate.per.class 
error$error.rate # Error rate of 0.63 over two components for OTU

# save data ####

# We will save the model to compare the error from integration to sPLS-DA.

#saveRDS(res.final, "HEU_manuscript_analysis/Rdata/R_export/blg_sgccda_res.rds")
